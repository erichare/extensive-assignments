...
Although the beta distribution cannot be written in terms of an exponential dispersion family, we will attempt to mimick GLMs in hopes of observing how a beta regression differs from a typical generalized linear model. To do so, we will use a link function which maps the mean $\mu$ onto the real line, a logit link, i.e.,

\begin{align*}
\mu_i &= h(x_i, \beta) \\
      &= \frac{e^{(\beta_0 + \beta_1x_i)}}{e^{(\beta_0 + \beta_1x_i)} + 1}
\end{align*}
