...
We discovered a package available on CRAN, pscl, which claims to be able to fit zero-inflated models for the data. We decided to compare the results obtained from our manual work and our own Newton-Raphson function to the results from this function, which claims to use optim underneath. The comparison is given in Table \ref{tbl:zip.compare}. The results match this package very closely, although they define the consumer interest parameter in the opposite manner. In other words, $\pi_i$ represents the probability of no consumer interest, while our interpretation is the opposite. This is why the pscl estimates of $\beta$ have opposite sign of ours. 
...
Finally, we consider the ZIP model within a Bayesian framework. We mainly did this out of curiosity to see if we could get Bayesian estimates that matched the MLEs.  Also, Bayesian estimation requires much less calculus than maximum likelihood estimation, so it would be nice to know that derivation of gradients and hessians as complicated as the ones we've encountered in this project can be avoided in the future. The model we consider is a straightforward Bayesian model for one store with noninformative priors on the model parameters.
\subsection{Prior Distributions}
  ...
  \end{align*}
  The code for our MCMC sampler is below.
<<metro.in.gibbs,eval=FALSE,echo=TRUE>>=
...
@
We used this sampler to estimate the parameter values for the first 9 stores, the same stores that we estimated the parameters of in sections 3 and 4. For each store, 3 chains were started at different points in the parameter space with 1,000 draws used for burn-in, and 10,000 draws used for inference. As an example, the code run for estimating the parameter values for the first store is given in the code appendix. The posterior distributions of all parameters are shown in Figure \ref{fig:bayes_plot}, and the posterior means and medians for these stores are listed in Table \ref{tbl:Bayes.ests}. 
<<sampler.example,echo=FALSE,eval=FALSE>>=
store1<-subset(beans,store==1009)
#burnin
burnin1 = run_mcmc(y=store1$mvm,x=store1$price,beta0=c(1,-1), delta0=c(1,-1),sigma.b=2,sigma.d=2,Sigma=matrix(c(100,0,0,100),nrow=2),tune=T)  
...
@
<<bayes_plot,echo=FALSE,warning=FALSE,fig.cap="Posterior Distributions for all 4 parameters at each of the first 9 stores in the data set">>=
mcmc_allstores<-read.table('mcmc_allstores.txt',header=T)
names(mcmc_allstores)<-c("delta0","delta1","beta0","beta1","Store")
library(gridExtra)
p1<-qplot(delta0,geom='histogram',binwidth=.01,data=mcmc_allstores,main='delta0')+facet_wrap(~Store)+theme(text = element_text(size=10))
p2<-qplot(delta1,geom='histogram',binwidth=.05,data=mcmc_allstores,main='delta1')+facet_wrap(~Store)+theme(text = element_text(size=10))
p3<-qplot(beta0,geom='histogram',binwidth=.1,data=mcmc_allstores,main='beta0')+facet_wrap(~Store)+theme(text = element_text(size=10))
p4<-qplot(beta1,geom='histogram',binwidth=.25,data=mcmc_allstores,main='beta1')+facet_wrap(~Store)+theme(text = element_text(size=10))
...
The Bayesian estimates for the parameters in the ZIP model are listed side-by-side with our MLEs for the ZIP model in Table \ref{tbl:bayes.mles.delt} and Table \ref{tbl:bayes.mles.bet}. 
<<bayes_mles,echo=FALSE,results='asis'>>=
names(zip.table)<-c("Store","b0_mle","b1_mle","d0_mle","d1_mle")
my.df<-merge(zip.table,bayes_ests,by='Store')
my.df<-my.df[c("Store"  , "mean_d0",  "med_d0" , "d0_mle", "mean_d1", "med_d1", "d1_mle", 
"mean_b0" ,"med_b0" , "b0_mle" ,"mean_b1", "med_b1", "b1_mle")]
print(xtable(my.df[,1:7], digits = c(0, 0, rep(4,6)), caption = "Comparison of Bayesian estimates of delta0 and delta1 to our MLEs", label = "tbl:bayes.mles.delt"), table.placement = 'H', include.rownames = FALSE)
print(xtable(my.df[,c(1,8:13)], digits = c(0, 0, rep(4,6)), caption = "Comparison of Bayesian estimates of beta0 and beta1 to our MLEs", label = "tbl:bayes.mles.bet"), table.placement = 'H', include.rownames = FALSE)
@
The Bayesian estimates of $\delta_0$ and $\delta_1$ are nearly identical to the MLEs; most of the posterior medians differ from the MLEs by less than 0.01. The Bayesan estimates of $\beta_0$ and $\beta_1$, however, are similar, but don't match the MLEs quite as closely.  Looking at the posterior distributions, we see that the distributions of the $\beta$s have much more variation than the distributions of the $\delta$s, which helps to explain why the $\beta$ Bayesian estimates don't match their respective MLEs as closely. Using more iterations for burnin and also for inference would help better estimate the posterior distributions of $\beta_0$ and $\beta_1$.   

\section{Conclusion}
In spite of the significant variation in sales across different stores, the ZIGP model appears to do a relatively good job of describing features in the data. The larger expected sales for high values of price, and larger tail probabilities in the actual ZIGP distribution better capture what the data itself tells us. Note that there is a significant limitations of this analysis, including the fact that we are treating daily sales as independent when some kind of covariance structure over time would likely be much more appropriate.

Furthermore, the Bayesian estimation of the ZIP model for individual stores showed us that if we do not have the time or the patience to derive all of the gradient and hessians for a future ZIP model, we can obtain parameter estimates through a Metropolis-within-Gibbs MCMC sampler that are just as accurate as the MLEs without the calculus headache.  The Bayesian model also has the advantage of being expandable by adding a hierarchical structure, so that new stores can be added and grouped by region or some other grouping variable, and the parameters for the whole group or region can be estimated just as easily as the parameters for an individual store.  
...
We tried to work together as much as possible, but we did a lot, so we obviously spent some time working individually and separately. Here is a rough outline of who did what (numbers refer to sections and/or subsections and two names indicate corrections/additions were made to the first person's work by the second person): 
\begin{itemize}
\item 1: Eric and Sam
\item 2: Eric
\item 3.1: Milli
\item 3.2.1: Milli and Eric
\item 3.2.2: Milli and Eric
\item 3.3: Eric
\item 4.1: Milli
\item 4.2.1: Milli and Sam
\item 4.2.2: Milli and Sam
\item 4.3: Eric
\item 5.1: Eric
\item 5.2.1: Sam and Eric
\item 5.2.2: Sam and Eric
\item 5.3: Eric
\item 6: Eric
\item 7 (all): Sam
\item 8: Eric and Sam
