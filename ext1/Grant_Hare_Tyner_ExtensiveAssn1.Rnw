...
The objective of this problem is to devise a model to describe all or a large part of the stores. We will first explore the data to see different behaviors. Then we shall develop a model for individual stores and a model the entire region as a whole. For both models we will assume that sales and price are each independent over time. In the process of exploration, we have determined that stores with ID 7000 and greater do not follow the same trends as the rest of the data. Therefore, we will exclude those stores from later analysis.

Figure \ref{fig:price_plot} displays histograms of the sales for each value of price in Store 1011. A few things are immediately clear in the data. First, stores price green beans at specific prices much more frequently than others. In particular, Store 1011 seemed to mostly price the green beans at either 0.68 or 0.77. Secondly, the zero-inflated nature of the data is very clear, with a large peak at zero sales.
...
Figure \ref{fig:sales_plot} displays boxplots of the daily sales for each of the first nine stores in the data. A few things become clear. First, the distributions are quite right-skewed, with some very large positive daily sales outliers. Second, the median sales are relatively low. There is also considerable variability between individual stores. For instance, STore 1013 appears heavily concentrated in the low-end of the sales spectrum, while Store 1019 has a much wider variation.
...
Now, define another set of random variables $\{Z_{i}: i = 1,...,n\}$ associated with consumer interest in green beans. Take these random variables to be independent and identically distributed with a binary probability mass function having parameter $\pi_i$. So,
\[Z_{i} = \left\{
  \begin{array}{lr}
...
&= (1 - \pi_i) + \frac{1}{0!}\lambda_i^{0}e^{-\lambda_i}\pi_i \\
&= (1- \pi_i) + \pi_ie^{-\lambda_i} \hspace{0.2 cm} \text{and} \\
...
\begin{align*}
L(\lambda_i, \pi_i) &= \prod_{i = 1}^n h(y_{i}| \lambda_i, \pi_i) \\
&= \prod_{i = 1}^n [(1 - \pi_i) + \pi_i e^{-\lambda_i}]I(y_{i} = 0) + [\frac{\pi_i e^{-\lambda_i}\lambda_i^{y_{i}}}{y_{i}!}]I(y_{i} > 0) \\
&= [(1 - \pi_i) + \pi_i e^{-\lambda_i}]I(y_{i} = 0) + [\pi_i e^{-\lambda_i} \lambda_i^{\sum^{n}_{i = 1}y_{i}} \prod_{i = 1}^{n} \frac{1}{y_{i}!}]I(y_{i} > 0) \\
\ell_{1}(\lambda_i, \pi_i) &= \bigg[\log{((1 - \pi_i) + \pi_i e^{-\lambda_i})}\bigg]I(y_{i} = 0) \\
\ell_{2}(\lambda_i, \pi_i) &= \bigg[\log{(\pi_i)} - \lambda_i + y_{i}\log{(\lambda_i)} - \log{(y_{i}!)}\bigg]I(y_{i} > 0) \\
\ell_i(\lambda_i, \pi_i) &= \ell_{1}(\lambda_i, \pi_i) + \ell_{2}(\lambda_i, \pi_i) \\
\ell(\lambda_i, \pi_i) &= \sum_{i=1}^n\ell_i(\lambda_i, \pi_i)
\end{align*}
Now, assume $\pi_i = \frac{e^{\beta_{0} + \beta_{1}x_{i}}}{1 + e^{\beta_{0} + \beta_{1}x_{i}}}$ and $\lambda_i = e^{\delta_{0} + \delta_{1}x_{i}}$. In other words, we will use a log link for the mean parameter of the poisson distribution, and a logit link for the consumer interest parameter of the zero-inflated portion of the distribution. This will define the systematic model component.
...
\subsection{Maximum Likelihood Estimation}
We will use our newton raphson function to obtain MLEs for the four regression parameters for the first nine stores in the dataset. The MLEs are given in Table \ref{tbl:zip.table}. A corresponding plot of the expectation function, where $E(Y_i) = \pi_i\lambda_i$, is given in Figure \ref{fig:zip.plot}.
...
qplot(Price, PredictedSales, data = zip.merge, geom = "line", facets=~Store, colour = I("blue"), size = I(2)) + geom_bar(aes(x = Price, y = actual_mvm), stat = "identity") + xlim(c(0.3, 0.75))
@
...
We want to develop a model that can take into account the extra variability that could come from the various days on which the covariate occurs. Therefore, we need a model that uses different Poisson distributions for the values of $Y_i$ that share a covariate value on several different days. This idea suggests the use of a Gamma-Poisson distribution. We formulate the model in the following section.
...
print(xtable(zigp.table, digits = c(0, 6, 6, 6, 6, 3), caption = "MLEs for the model for all stores", label = "tbl:zigp.table"), table.position = 'H')
@
...
print(xtable(my.df, digits = c(0, rep(6, 8)), caption = "Comparison of MLEs to results obtained from pscl package", label = "tbl:zigp.compare"), table.position = 'H')
@
