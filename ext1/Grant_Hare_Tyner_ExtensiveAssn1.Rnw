...
Furthermore, the Bayesian estimation of the ZIP model for individual stores showed us that if we do not have the time or the patience to derive all of the gradient and hessians for a future ZIP model, we can obtain parameter estimates through a Metropolis-within-Gibbs MCMC sampler that are just as accurate as the MLEs without the calculus headache.  The Bayesian model also has the advantage of being expandable by adding a hierarchical structure, so that new stores can be added and grouped by region or some other grouping variable, and the parameters for the whole can be estimated just as easily as the parameters for an individual store.  
